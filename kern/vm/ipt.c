...
//static unsigned int queue_fifo[IPT_SIZE];
...
void ipt_map(pid_t pid, vaddr_t v, paddr_t p){
    ...
    spinlock_acquire(&free_ipt);
    paddr_t res = ipt_swap_pop(&ipt[i]); /* pop on swapfile if paddr is written there */
...
paddr_t ipt_translate(vaddr_t v){
    unsigned i;
...
paddr_t ipt_swap_pop(struct ipt_entry* ipt_e){
    paddr_t res = 0;
    (void)ipt_e;
    //res=swap_out(ipt_e->paddr);
