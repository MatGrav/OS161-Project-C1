...
Un repository su GitHub è stato utilizzato per tenere traccia di tutti i cambiamenti ed introduzioni nel codice. Ciò ha permesso ai membri del gruppo di lavorare in linea generale su computer separati e in autonomia, consentendo una parziale suddivisione del lavoro. Tuttavia, la natura interconnessa del codice ha richiesto ai membri del gruppo di confrontarsi frequentemente sui lavori da farsi, per essere sempre aggiornati sulle funzioni implementate, sulle problematiche comuni e su come esse comunichino con altri nuovi moduli di OS161.
Durante le prime fasi di progettazione, si sono scelti alcuni test da eseguire, la cui lista è stata rivista e consolidata per la fase finale di raccolta delle statistiche, riportate nell'ultimo paragrafo del documento.
...
La coremap è vista come una struttura dati che ha all'interno: un array di struct di entry generica, la size della coremap, un vettore di pagine libere e la sua size.  Della entry generica si evidenziano i campi relativi all'indirizzo fisico della pagina e il campo di stato, se occupato, libero o riservato dal kernel. 
...
La ricerca all'interno della page table è gestita da una funzione statica chiamata `ipt_search()`. Tale funzione, chiamando `ipt_mra` (Most Recently Added), ottiene _BUFF_SIZE_ indici di pagina in cui verifica se l'elemento cercato è presente in tali pagine. Poiché il buffer utilizzato per contenere gli indici li ottiene prelevandoli dalla coda di __queue_fifo__, allora si cercherà l'elemento prima nelle più recenti pagine aggiunte.  
In una seconda fase, qualora non avesse ancora trovato l'elemento cercato, farà una ricerca lineare all'interno della IPT.
...
## Strumentazione ed esito dei test
